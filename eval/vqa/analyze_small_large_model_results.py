...
def evaluate_chat_model():
    ...
    for ds_name in args.datasets:
        ...
        small_model_results = json.load(open("/mnt/nas_jiasheng/zhaowangbo/early_exit_vl_CVPR2025/InternVL/results/1B/textvqa_val_240728152644.json", "r"))
        large_model_results = json.load(open("/mnt/nas_jiasheng/zhaowangbo/early_exit_vl_CVPR2025/InternVL/results/2B/textvqa_val_240728160238.json", "r"))
        ...
        if ds_collections[ds_name]['metric'] == 'vqa_score':
            ...
            for item1, item2 in zip(small_model_results, large_model_results):
                item1['pred_answer'] = item1['answer']
                item1['gt_answers'] = question_id2answers[item1['question_id']]
                
                item2['pred_answer'] = item2['answer']
                item2['gt_answers'] = question_id2answers[item2['question_id']]
            
                
            
            small_model_pred_scores = evaluator.eval_pred_scores(small_model_results)
            large_model_pred_scores = evaluator.eval_pred_scores(large_model_results)
            
            
            import numpy as np
            small_model_pred_scores = np.array(small_model_pred_scores)
            large_model_pred_scores = np.array(large_model_pred_scores)
            
            print(f"small > large {(small_model_pred_scores > large_model_pred_scores).mean()}")
            print(f"small < large {(small_model_pred_scores < large_model_pred_scores).mean()}")
            print(f"small == large {(small_model_pred_scores == large_model_pred_scores).mean()}")
