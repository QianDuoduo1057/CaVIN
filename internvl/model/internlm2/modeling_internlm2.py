...
class InternLM2Model(InternLM2PreTrainedModel):
    ...
    def forward(
        ...
        retention_ratio_after_prune = 1.0
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if token_prune and idx == K:
                    device = hidden_states.device
                    visual_token_length = visual_token_index[1] - visual_token_index[0] + 1
    
                    if visual_token_importance is None:
                        raise ValueError("visual_token_importance must be provided for token pruning.")
    
                    if isinstance(visual_token_importance, list):
                        binary_mask = torch.tensor(visual_token_importance, device=device)   # 二值掩码，1表示保留，0表示剪枝
                    else:
                        binary_mask = visual_token_importance.to(device)
    
                    keep_visual_indices = torch.nonzero(binary_mask == 1, as_tuple=True)[0]
                    
                    if hidden_states.shape[1] != 1:
                        keep_visual_token_indices = keep_visual_indices + int(visual_token_index[0])
    
                        keep_indexs = torch.cat((
                            torch.arange(int(visual_token_index[0]), device=device),
                            keep_visual_token_indices,
                        ...
                        if attention_mask is not None:
                            ...
                            attention_mask = attention_mask[:, :, :new_seq_len, :new_seq_len]   # 同步裁剪mask
    
                        kept = keep_visual_indices.shape[0]
                        prunded_sequence_length = int(visual_token_length) - kept
                        retention_ratio_after_prune = kept / int(visual_token_length)
                    
                    elif hidden_states.shape[1] == 1:
                        kept = int(torch.sum(binary_mask == 1).item())
                        prunded_sequence_length = int(visual_token_length) - kept
                        retention_ratio_after_prune = kept / int(visual_token_length)
                ...
                layer_outputs = decoder_layer(
                    ...
                    output_attentions=output_attentions,
                    use_cache=use_cache,
                    prunded_sequence_length=prunded_sequence_length if token_prune and idx >= K else 0
                )
        ...
        if token_prune:
            avg_retention = (K * 1.0 + (total_layers - K) * retention_ratio_after_prune) / total_layers
            if seq_length > 1:
                print(f"[Token Pruning] avg_retention_ratio={avg_retention:.2%}")
