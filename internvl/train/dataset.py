...
def dynamic_preprocess(image, min_num=1, max_num=6, image_size=448, use_thumbnail=False):   # 统一图片大小
    orig_width, orig_height = image.size    # 获取原始图片大小
    ...
    return processed_images # 最后一张才是缩略图
...
import numpy as np
from PIL import Image
def save_processed_images(processed_images, output_dir="processed_images", prefix="img"):
    """
    将processed_images保存到本地文件
    """
    import os
    ...
    filename = f"CoTImage.png"
    filepath = os.path.join(filename)
    processed_images[4].save(filepath)
