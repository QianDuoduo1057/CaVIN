...
def CausalInferencePredictor(x, y, model_path, model_width=2, input_dim=512,
                             device='cuda', threshold=0.15,
                             auto_pad=True, pad_value=0):
    """
    批量 visual token 因果方向预测。

    Args:
        x, y:        shape (D,) 或 (B, D)，会自动升维到 (B, D)
        model_path:  checkpoint 路径
        model_width: 模型宽度乘子
        input_dim:   输入维度 / 自动填充目标长度
        device:      推理设备
        threshold:   分类阈值，'auto' 则用 Otsu 自动确定
        auto_pad:    是否自动填充/截断到 input_dim
        pad_value:   填充值

    Returns:
        dict: raw_output, threshold_used, binary_mask, labels, descriptions, confidence
    """
    ...
    with torch.no_grad():
        outputs = model(x_tensor, y_tensor)          # (B,)

    if outputs.dim() == 0:
        outputs = outputs.unsqueeze(0)

    raw_np = outputs.cpu().numpy()                    # (B,)
    ...
    # 绝对值 >= 阈值 → 1（不确定 / 无因果），否则 → 0
    binary_mask = (np.abs(raw_np) >= threshold_used).astype(np.int32)

    labels = [_output_to_label(v, threshold_used) for v in raw_np]
