...
class CausalDataset(Dataset):
    ...
    def __init__(self, file_path, sample_indices=None):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            results = list(executor.map(safe_json_loads, valid_lines))
        # 过滤和预处理数据
        for i, item in enumerate(results):
            if sample_indices is None or i in sample_indices:
                if item is None:
                    print("出现错误数据集，错误行数为第" + str(i) + "行")
                    continue
                item['reason'][255], item['result'][255] = 0.1, 0.1
                question_reason, question_result = item['reason'][256:], item['result'][256:]
                item['reason'], item['result'] = item['reason'][0:256], item['result'][0:256]
                x = [val if val >= 2 else 0.1 for val in item['reason']]
                y = [val if val >= 2 else 0.1 for val in item['result']]
                if isinstance(question_reason, int):
                    question_reason = [question_reason]
                if isinstance(question_result, int):
                    question_result = [question_result]
                x.extend(question_reason)
                y.extend(question_result)
                # import pdb; pdb.set_trace()
